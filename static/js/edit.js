...
function drawPhotoStrip() {
    ...
    canvas.width = 700;
    canvas.height = isMobile ? 2400 : 2100; 
    
    const photoCount = capturedPhotos.length;
    const photoWidth = 600; // Leave some margin on sides like classic photobooth
    const photoHeight = isMobile ? 650 : 500; // Increase individual photo height for mobile (30% larger)
    const spacing = isMobile ? 100 : 80; // More space between photos on mobile
